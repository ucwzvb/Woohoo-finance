...
with st.container():
    ...
    with col2:
        ...
        st.markdown("The Stock Price in Trade", unsafe_allow_html=True)
...
with st.container():
    ...
    with col1:
        st.write('## Key data')
        st.write('''
        | Previous Close |       1,084.60      |        Market Cap        |            1.019T           |
        |:--------------:|:-------------------:|:------------------------:|:---------------------------:|
        |      Open      |       1,084.79      |     Beta (5Y Monthly)    |             2.01            |
        |       Bid      |    1,008.00 x 900   |      PE Ratio (TTM)      |            331.47           |
        |       Ask      |   1,008.50 x 1800   |         EPS (TTM)        |             3.06            |
        |   Day's Range  | 1,000.21 - 1,090.58 |       Earnings Date      | Jan 25, 2022 - Jan 31, 2022 |
        |  52 Week Range |  539.49 - 1,243.49  | Forward Dividend & Yield |          N/A (N/A)          |
        |     Volume     |      30,516,306     |     Ex-Dividend Date     |             N/A             |
        |   Avg. Volume  |      25,463,979     |       1y Target Est      |            836.72           |''')
    with col2:
        trace1 = go.Bar(
            x=earn.index,
            y=earn.Revenue,
            name="Revenue",
            marker=dict(color='rgb(0, 192, 115)', line=dict(color='rgb(255,255,255)', width=0)),
            text="Revenue")
        # 构造 trace2
        trace2 = go.Bar(
            x=earn.index,
            y=earn.Earnings,
            name="Earnings",
            marker=dict(color='rgb(24, 143, 255)', line=dict(color='rgb(255,255,255)', width=0)),
            text="Earnings")
        data = [trace1, trace2]
        import plotly.io as pio

        pio.templates.default = "none"
        layout = go.Layout(barmode="group", template="none", width=800, height=450, title='company finicials', )
...
if opt_E_A == 'European':
    ...
    for K in df_c['strike']:
        ...
        premium_call_mcs = mcs_option_price(lcp, K, uty, T, sigma, 'C', 5000000)
        premium_call_FFT = BSM_call_value_FFT(lcp, K, uty, T, sigma)
        premium_call_fd = FD_option_value(M=5000, S=lcp, K=K, r=uty, sigma=sigma, delta_S=3 * lcp / 5000,
                                          delta_T=T / 5000, type='C')
        ...
        option_df_c = option_df_c.append(
            {
                ...
                'finite difference method': premium_call_fd,
    ...
    for K in df_p['strike']:
        ...
        premium_put_crr = CRR_option_value(lcp, K, uty, T, sigma, 'P', round(T * 360 * 2))
        premium_put_mcs = mcs_option_price(lcp, K, uty, T, sigma, 'P', 5000000)
        premium_call_FFT = BSM_call_value_FFT(lcp, K, uty, T, sigma)
        premium_put_fd = FD_option_value(M=500, S=lcp, K=K, r=uty, sigma=sigma, delta_S=3 * lcp / 500,
                                         delta_T=T / 500, type='P')
        ...
        option_df_p = option_df_p.append(
            {
                ...
                'finite difference method': premium_put_fd,
                'Fourier-based approach': premium_call_FFT,
                'Difference': difference_iv,
                'Volume': df_p.loc[df_p['strike'] == K, 'volume'].iloc[0],
    ...
    if type_C == 'Call option':
        ...
        option_df_c.index = option_df_c['Expiration']
        df_1 = option_df_c[['Premium', 'Black-Scholes', 'Cox-Ross-Rubinstein', 'Monte Carlo Simulation','finite difference method','Fourier-based approach']]
        last_rows = pd.DataFrame(df_1.iloc[0]).transpose()
        chart = st.line_chart(last_rows, width=0, height=355)
        ...
        option_df_p.index = option_df_p['Expiration']
        df_1 = option_df_p[
            ['Premium', 'Black-Scholes', 'Cox-Ross-Rubinstein', 'Monte Carlo Simulation', 'finite difference method',
             'Fourier-based approach']]
        last_rows = pd.DataFrame(df_1.iloc[0]).transpose()
        chart = st.line_chart(last_rows, width=0, height=355)
    ...
    for K in df_p['strike']:
        ...
        premium_put_mcs = mcs_option_price(lcp, K, uty, T, sigma, 'P', 5000000)
        premium_call_FFT = BSM_call_value_FFT(lcp, K, uty, T, sigma)
        premium_put_fd = FD_option_value(M=5000, S=lcp, K=K, r=uty, sigma=sigma, delta_S=3 * lcp / 5000,
                                         delta_T=T / 5000, type='P')
        difference_iv = market_premium - premium_put_iv
        option_df_p_Eu = option_df_p_Eu.append(
            {
                'Contract': df_p.loc[df_p['strike'] == K, 'contractSymbol'].iloc[0],
                'Expiration': Date,
                'Price': lcp,
    ...
    for K in df_p['strike']:
        ...
        volume = df_p.loc[df_p['strike'] == K, 'volume'].iloc[0]
        inTheMoney = df_p.loc[df_p['strike'] == K, 'inTheMoney'].iloc[0]
        premium_put_crr = CRR_option_valuation_Am(lcp, K, uty, T, sigma, round(T * 360 * 2))
        premium_put_mcs = LSM(lcp, K, uty, T, sigma, 50000, round(T * 360))
        premium_put_BAW = getValue('American', 'Value', 'Put', lcp, K, T, uty, uty, sigma)
        [v0_fidi_am, s0_fidi_am] = fidi_bs_american(K, uty, sigma, T, -3 * uty, 3 * uty, 5000, 5000)
        spot = lcp  # list of spot prices
        v0_fidi_am_interp = np.interp(spot, s0_fidi_am,
                                      v0_fidi_am)  # find the price for different spots via linear interpolation
        option_df_p = option_df_p.append(
            {
                'Contract': df_p.loc[df_p['strike'] == K, 'contractSymbol'].iloc[0],
                'Expiration': Date,
                ...
                'Cox-Ross-Rubinstein': premium_put_crr,
                'Monte Carlo Simulation': premium_put_mcs,
                'finite difference method': v0_fidi_am_interp,
                'Volume': df_p.loc[df_p['strike'] == K, 'volume'].iloc[0],
                'Open Interest': df_p.loc[df_p['strike'] == K, 'openInterest'].iloc[0],
                'Impelied Volatility': df_p.loc[df_p['strike'] == K, 'impliedVolatility'].iloc[0],
            },
